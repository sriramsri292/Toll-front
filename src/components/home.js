...
useEffect(() => {
  ...
  const backend = async () => {
    try {
      ...
      const response = await fetch('https://pathfinal.onrender.com/tool/api', {
        method: 'POST',
        headers: {
...
const sendRequestToBackend = async () => {
  try {
    ...
    const response = await fetch('https://pathfinal.onrender.com/tool/api/tollguru/v1', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-api-key': 'NLFQj8MDN7bhHnHgpmPNLnBJQ8HTjdLQ',
      },
      body: JSON.stringify(payload),
    });
...
<div> 
<Button className="button" onClick={handleButtonClick} variant="success">Calculate Tolls</Button>{' '}
    <Container > 
